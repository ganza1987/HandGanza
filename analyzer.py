...
async def build_real_data(home_name: str, away_name: str) -> dict:
    ...
    print(f"[DEBUG] build_real_data({home_name}, {away_name}) -> api_ok={api_ok} home_found={ht is not None} away_found={at is not None}")
    return match_data
...
async def analyze_match(home: str, away: str, conditions: list[dict] | None = None) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=120) as client:
            ...
            if not text_parts:
                print(f"[DEBUG] Respuesta de Anthropic sin texto. content={response.get('content')}")
